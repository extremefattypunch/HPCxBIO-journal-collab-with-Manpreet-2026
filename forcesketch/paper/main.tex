...
A profile at $B{=}16$ confirms the mechanism: the shared forward is only $9.4$\,ms of a
$100.6$\,ms exact step, so the reverse pass dominates by roughly an order of magnitude and each
removed lane removes $\sim\!12$\,ms of it. We report an important caveat in Section~\ref{sec:limitations}: our exact baseline
