...
($10^3$ resamples), computed on the seed-averaged statistic; seed-to-seed standard deviations are
...
Yes, and remarkably uniformly. The measured ceiling on incremental UQ speedup,
$T(L{=}8)/T(L{=}K{+}1)$, is $\fsCeilingKThreeBOne\times$ at $K{=}3$ and $\fsCeilingKTwoBOne\times$
at $K{=}2$. Sweeping batch sizes $B\in\{1,4,16,64\}$ and three molecules spanning 9 to 27 atoms,
that ceiling stays within $1.77$--$1.87\times$ at $K{=}3$ and $2.17$--$2.39\times$ at $K{=}2$. This
answers spec-level question of what the saving actually depends on: \emph{the reverse-lane count},
not the atom count and not the batch size. Real MACE is compute-bound even at $B{=}1$ (a single
lane costs $\sim\!21$\,ms for 27 atoms), so the launch-bound regime in which extra cotangent lanes
would ride along free never occurs on this hardware. We report an important caveat in Section~\ref{sec:limitations}: our exact baseline
